...
def compute_wer(hyps, refs):
    # 유효한 참조 텍스트만 필터링
    valid_pairs = [(hyp, ref) for hyp, ref in zip(hyps, refs) if ref and len(ref.strip()) > 0]
    
    if not valid_pairs:
        print("No valid reference texts for WER calculation.")
        return None
    ...
    wer_list = []
    for hyp, ref in zip(valid_hyps, valid_refs):
        normalizer = EnglishTextNormalizer()

        norm_ref = normalizer(ref)
        norm_hyp = normalizer(hyp)
        
        distance = ed.eval(norm_ref.split(), norm_hyp.split())
        ref_length = len(norm_ref.split())
...
def compute_spider(hyps, refs):
    # 유효한 참조 텍스트만 필터링
    valid_pairs = [(hyp, ref) for hyp, ref in zip(hyps, refs) if ref and len(ref.strip()) > 0]
    
    if not valid_pairs:
        print("No valid reference texts for SPIDEr calculation.")
        return None
    ...
    try:
        # SPIDEr 계산을 위해 참조 텍스트 포맷 변환
        refs_for_spider = [[ref_] for ref_ in valid_refs]
        spider_result = spider(candidates=valid_hyps, mult_references=refs_for_spider)
        spider_score = round(float(spider_result[0]['spider']), 4)
        print(f"SPIDEr: {spider_score}")
        return spider_score
    except Exception as e:
        print(f"Error in SPIDEr calculation: {e}")
        return None