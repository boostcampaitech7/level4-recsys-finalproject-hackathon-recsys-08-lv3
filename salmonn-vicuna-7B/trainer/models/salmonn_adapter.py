...
from peft import LoraConfig, TaskType, get_peft_model, AdapterConfig
...
class SALMONN(nn.Module):
    ...
    def __init__(
        ...
        if not only_preprocessor:
            ...
            if self.adapter:
                self.peft_config = AdapterConfig(
                    task_type=TaskType.CAUSAL_LM,
                    inference_mode=False,
                    adapter_hidden_size=self.adapter_config.adapter_hidden_size,
                    adapter_size=self.adapter_config.adapter_size,
                    dropout=self.adapter_config.dropout,
                    non_linearity=self.adapter_config.non_linearity,
                    target_modules=["query", "value"]
                )
                self.llama_model = get_peft_model(self.llama_model, self.peft_config)
                self.llama_model.print_trainable_parameters()
                logging.info('Adapter Training')
    ...
    def prompt_wrap(self, embeds, atts, prompt, multi_prompt=False):
        if prompt:
            if multi_prompt:
                ...
                p_before_embeds = self.llama_model.model.embed_tokens(p_before_tokens.input_ids) if not self.adapter else self.llama_model.model.model.embed_tokens(p_before_tokens.input_ids)
                ...
                p_after_embeds = self.llama_model.model.embed_tokens(p_after_tokens.input_ids) if not self.adapter else self.llama_model.model.model.embed_tokens(p_after_tokens.input_ids)
                ...
                p_before_embeds = self.llama_model.model.embed_tokens(p_before_tokens.input_ids).expand(batch_size, -1, -1) if not self.adapter else self.llama_model.model.model.embed_tokens(p_before_tokens.input_ids).expand(batch_size, -1, -1)
                p_after_embeds = self.llama_model.model.embed_tokens(p_after_tokens.input_ids).expand(batch_size, -1, -1) if not self.adapter else self.llama_model.model.model.embed_tokens(p_after_tokens.input_ids).expand(batch_size, -1, -1)
    ...
    def forward(self, samples, verbose=False):
        ...
        to_regress_embeds = self.llama_model.model.embed_tokens(to_regress_tokens.input_ids) if not self.adapter else self.llama_model.model.model.embed_tokens(to_regress_tokens.input_ids)
        ...
        bos_embeds = self.llama_model.model.embed_tokens(bos) if not self.adapter else self.llama_model.model.model.embed_tokens(bos)
    ...
    def from_config(cls, config):
        ...
        adapter = config.get("adapter", True)
