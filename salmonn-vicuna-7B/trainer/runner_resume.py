...
class Runner:
    def __init__(self, cfg, model, datasets, job_id, dryrun, start_epoch=0):
        self.config = cfg
        self.job_id = job_id
        self.dryrun = dryrun
        ...
        # log
        self.output_dir = Path(self.config.config.run.output_dir) / job_id # "outputs_stage1_only/2025-01-23-00:00:00" 
        self.output_dir.mkdir(parents=True, exist_ok=True) 
        self.log_writter = SummaryWriter(self.output_dir) # 이 기능을 통해 학습과정 및결과를 시각화 가능

        # settings
        self.device = torch.device(self.config.config.run.device)
        self.use_distributed = self.config.config.run.use_distributed # use_distrbuted => 데이터 parrlell
        self.max_epoch = self.config.config.run.optims.max_epoch
        self.evaluate_only = self.config.config.run.evaluate # True or False 로 test 데이터에서 evaluate 진행
        self.cuda_enabled = (self.device.type == "cuda") #  True or False 로나옴
        ...
        if test_prompt_path: # test_prompt_path 읽음 
            try:
                ...
                with open(test_prompt_path, "r", encoding="utf-8") as f:
                    self.test_prompt_dict = json.load(f)
            for k in self.test_prompt_dict.keys(): # keys() ==> test_prompt의 task들 그러면 self.test_prompt_dict[k] 는 그 테스크에 맞는 프롬트
                self.test_prompt_dict[k] = self.prompt_template.format(self.test_prompt_dict[k])
                # prompt_template = "USER: {}\nASSISTANT:"
                # test_prompt = "Hello, how are you?"

                # formatted_prompt = prompt_template.format(test_prompt)
        ...
        if self.use_distributed:
            self.model = DDP(
                self._model, device_ids=[self.config.config.run.gpu] # 여기 고쳐야함 2개면 [0,1]이런식으로
            )
        else:
        ...
        # numerical stability를 위한 computation??
        if self.use_amp:
            self.scaler = torch.cuda.amp.GradScaler()
        else:
            self.scaler = None
        # self.use_amp  autocast에쓰임
        # self.scaler 는 loss.backward()에쓰임 => FP16 underflow 가는걸 막기위해 scale up! loss 가 너무낮을시를 방지하는역할

        # optimizer & scheduler
        self.iters_per_epoch = len(self.train_loader) if self.config.config.run.epoch_based else self.config.config.run.iters_per_epoch # trainstage1 에서는 iters_per_epoch => 3000
        self.optimizer = get_optimizer(self.model, self.config.config.run.optims)
        self.scheduler = LinearWarmupCosineLRScheduler(
            self.optimizer,
            max_epoch=self.max_epoch, # 30
            iters_per_epoch=self.iters_per_epoch, # stage 1 에서는 3000
            min_lr=self.config.config.run.optims.min_lr,
            init_lr=self.config.config.run.optims.init_lr,
            warmup_steps=self.config.config.run.optims.warmup_steps,# warmup_Steps 3000
    ...
    def train_epoch(self, epoch):
        ...
        for i in metric_logger.log_every(range(self.iters_per_epoch), self.config.config.run.log_freq, header=header, logger=self.log_writter , start_step=epoch*self.iters_per_epoch):
            ...
            if i >= self.iters_per_epoch:
                break
            # next 로 self.train_loader 샘플 부름
            ...
            if not self.dryrun:
                ...
                if self.use_distributed:
                    ...
                    global_rank = 0  # Default for single-GPU
    ...
    def train(self):
        ...
        # start_epoch부터 max_epoch까지
        for cur_epoch in range(self.start_epoch + 1, self.max_epoch):
