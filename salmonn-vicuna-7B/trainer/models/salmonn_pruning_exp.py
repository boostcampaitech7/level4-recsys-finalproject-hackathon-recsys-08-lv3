...
class SALMONN(nn.Module):
        ...
        if not only_preprocessor:
            ...
            if self.lora:
                self.peft_config = LoraConfig(
                    task_type=TaskType.CAUSAL_LM,   # task_type=LoRA가 적용될 task의 유형 지정
                    inference_mode=False,           # 모델의 동작 모드 지정(학습모드, 추론모드)
                    # QLoRA 파라미터 설정(rank, alpha, dropout)
                    r=lora_rank, 
    ...
    def forward(self, samples, verbose=False, num_layers_to_keep=None):
        # 새로운 인자 num_layers_to_keep 추가
        if num_layers_to_keep is not None:
            # LLaMA 모델의 layers를 pruning
            self.llama_model.model = self.llama_model.model.prune_layers(num_layers_to_keep)
