...
class SALMONN(nn.Module):
    ...
    def __init__(
        ...
        if not only_preprocessor:
            ...
            if self.lora:
                ...
                self.peft_config = LoraConfig(
                    ...
                    # QLoRA 추가 설정
                    target_modules=["q_proj", "v_proj"],  # 타겟 모듈 지정
                    bias="none",
        ...
        # 음성 특징(Whisper) 및 비음성 특징(BEATs)의 Layer Normalization 적용을 위한 초기화 -> 체크포인트(.pth) 파일에서 로드 시 실제 값 할당
        # Whisper 인코더를 위한 Layer Normalization
    ...
    def encode_speech(self, spectrogram, raw_wav=None, audio_padding_mask=None):
        with self.maybe_autocast():
            ...
            # BEATs로 음성 특징 추출 (raw_wav가 있는 경우)
    ...
    def forward(self, samples, verbose=False):
        # 1. 태스크 확인 및 프롬프트 선택 - detect whether there are multi tasks in this batch
        task = list(set(samples["task"]))
        if len(task) > 1 or "QA" in task:
            self.multi_prompt = True

        # prepare prompts randomly
        if self.prompt_dict:
        ...
        # 2. 음성 데이터 인코딩 - use speech/audio encoder to encode speech/audio
        spectrogram = samples["spectrogram"]    # 입력 음성을 스펙트로그램으로 변환
        raw_wav = samples.get("raw_wav", None)   # 입력 음성 데이터
        audio_padding_mask = samples.get("padding_mask", None)  # 음성 데이터의 패딩 마스크

        speech_embeds, speech_atts = self.encode_speech(spectrogram, raw_wav=raw_wav, audio_padding_mask=audio_padding_mask)    # 음성 데이터를 인코딩하여(from BEATs and Whisper) 임베딩 벡터 생성

        # wrap speech_embeds with prompts
        if self.prompt_dict:
            speech_embeds, speech_atts = self.prompt_wrap(speech_embeds, speech_atts, prompt, multi_prompt=self.multi_prompt)    # 프롬프트와 음성 특징 결합 -> 프롬프트를 사용하여 임베딩 벡터 처리

        # 3. 텍스트 처리 및 모델 입력 준비 - prepare inputs for LLM
        text = [t + self.end_sym for t in samples["text"]]    # 텍스트 끝에 종료 토큰 추가
        to_regress_tokens = self.llama_tokenizer(   # 텍스트를 토큰화하여 텐서로 변환
            text,
            return_tensors="pt",    # 텍스트를 토큰화하여 텐서로 변환
            padding="longest",      # 가장 긴 텍스트 길이에 맞춰 패딩
            truncation=True,        # 텍스트 길이가 max_length를 초과하면 잘라냄
        ...
        with self.maybe_autocast():
            ...
            loss = outputs.loss
        # 5. 예측 결과 계산(토큰 단위, verbose=True인 경우)
        if verbose:
