...
def get_dataloader(dataset, config, is_train=True, use_distributed=True):
    ...
    loader = DataLoader(
        ...
        pin_memory=True,
        sampler=sampler,
        shuffle=sampler is None and is_train, # 앞에 DistributedSample shuffle에서 이미 셔플됨
        collate_fn=dataset.collater,
        drop_last=is_train,
    )

    if is_train:
        loader = IterLoader(loader, use_distributed=use_distributed) #IterLoader는 epoch 으로 훈련시키는것을 의미없게만들수있음
    ...
    def __next__(self):
        try:
            ...
            if hasattr(self._dataloader.sampler, "set_epoch") and self._use_distributed:
                ...
                # 한번 gpu 에 배치된 batches들은 다음 epoch때 똑같이 유지되지만 셔플은한다 이 셔플 순서를 다른 gpu에도 공유하기위한 기능!
